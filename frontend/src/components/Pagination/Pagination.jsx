...
const {totalPages,page,setPage} = useProductContext();
  ...
  return (
    <div className='flex justify-center items-center gap-2 bg-gray-200 p-2 rounded-md '>
      <CustomButton className={`bg-sky-400 rounded font-bold`} onClick={()=>setPage(p=>p-1)}>Prev</CustomButton>
      {totalP.map((p,index)=>{
      ...
      <CustomButton className={`bg-sky-400 rounded font-bold`} onClick={()=>setPage(p=>p+1)}>Next</CustomButton>
    </div>
  )
}
